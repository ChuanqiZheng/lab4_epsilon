...
#include <lab4_epsilon/lab4Action.h>
...
private:
    ...
    actionlib::SimpleActionServer<lab4_epsilon::lab4Action> as_;
    ...
    lab4_epsilon::lab4Goal goal_; // goal message, received from client
    lab4_epsilon::lab4Result result_; // put results here, to be sent back to the client when done w/ goal
    lab4_epsilon::lab4Feedback feedback_; // for feedback
...
public:
    ...
    // Action Interface
    void executeCB(const actionlib::SimpleActionServer<lab4_epsilon::lab4Action>::GoalConstPtr& goal);
};
...
void do_inits(ros::NodeHandle &nh_) {
  //initialize components of the twist command global variable
    ...
    g_twist_commander = nh_.advertise<geometry_msgs::Twist>("mobile_base/commands/velocity", 1);  
}
...
void ExampleActionServer::executeCB(const actionlib::SimpleActionServer<lab4_epsilon::lab4Action>::GoalConstPtr& goal) {
    ...
    while(timer<final_time) {
          ...
          {
             do_halt();
             ROS_INFO("Lidar alarm received, halt!");
             while(1)
             {
                ros::spinOnce();
                if(!g_lidar_alarm)
                {
                   ROS_INFO("Lidar alarm dissapear, keep going!");
                   g_twist_cmd.angular.z= sgn(spin_angle)*g_spin_speed;
                   break;
                }
                loop_timer.sleep();
